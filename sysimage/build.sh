...
julia --startup-file=no --output-o sysimage/sys.o -J$DEF_PATH sysimage/custom_sysimage.jl $STDLIB_PATH > log.txt
echo "Creating sysimage shared library.."
gcc -shared -o sysimage/sys.so -Wl,--whole-archive sysimage/sys.o -Wl,--no-whole-archive -L$JULIA_LIB_PATH -ljulia >> log.txt
rm sysimage/sys.o
